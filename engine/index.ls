...
backend = do
  ...
  init: -> new Promise (res, rej) ~>
    ...
    app.use aux.custom-error body-parser.json(limit: config.limit), 1008
    app.use aux.custom-error body-parser.urlencoded(extended: true, limit: config.limit), 1008
    ...
    if !config.debug =>
      ...
      if err.code == \EBADCSRFTOKEN =>
        ...
        # ignore user data parsing error 1008
        try
          msg = err.toString! or err.message
        catch e
          msg = ""
        if err instanceof lderror and err.id == 1008 =>
          console.error(
            colors.red.underline("[#{moment!format 'YY/MM/DD HH:mm:ss'}]"),
            colors.yellow("[ldError/1008]"),
            colors.yellow(err.message),
            "[", colors.yellow(req.originalUrl.substring(0,15)), "]"
          )
        ...
        # ignore some errors that we don't need to take care.
          ...
          return res.status 400 .send!
        else if /^(FacebookTokenError|TokenError|FacebookAuthorizationError)/.exec(msg) =>
          console.error(
            colors.red.underline("[#{moment!format 'YY/MM/DD HH:mm:ss'}]"),
            colors.yellow(err.message)
            "[", colors.yellow(req.originalUrl.substring(0,15)), "]"
          )
        else if msg.startsWith 'Error: Failed to lookup view' =>
          console.error(
            colors.red.underline("[#{moment!format 'YY/MM/DD HH:mm:ss'}]"),
            colors.yellow(msg)
          )
