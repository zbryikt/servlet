...
base = do
  ...
  # wrap middleware to rewrite error for better error handling
  # rewrite = either:
  # - int: a number for lderror id
  # - func: a function accepting error as param and return a custom error.
  custom-error: (func, rewrite = 0) -> (req, res, next) ->
    _next = ->
      if it and (it instanceof Error) =>
        next if typeof(rewrite) == \function => rewrite(it) else new lderror(it, rewrite)
      else next!
    try
      func req, res, _next
    catch e
